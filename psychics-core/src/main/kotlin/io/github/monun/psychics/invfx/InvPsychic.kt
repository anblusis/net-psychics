...
object InvPsychic {
    ...
    fun create(psychicConcept: PsychicConcept, stats: (EsperStatistic) -> Double, guide: Boolean = false): InvFrame {
        return InvFX.frame(1, text("PSYCHICS")) {
            ...
            list(2, 0, 6, 0, true, { psychicConcept.abilityConcepts }) {
                transform { it.renderTooltip(stats, guide).applyTo(it.wand ?: ItemStack(Material.BOOK)) }
                if (!guide) {
