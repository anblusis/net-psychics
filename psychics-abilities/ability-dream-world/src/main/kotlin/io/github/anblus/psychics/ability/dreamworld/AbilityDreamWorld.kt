...
class AbilityDreamWorld : ActiveAbility<AbilityConceptDreamWorld>(), Listener {
    ...
    inner class RainbowProjectile : PsychicProjectile(12000, concept.range) {
        lateinit var rainbow: FakeEntity<ArmorStand>
            ...
            trail.velocity?.let { v ->
                ...
                if (length > 0.0) {
                    ...
                    world.rayTrace(
                        ...
                        remove()
                        world.spawnFirework(block.location, FireworkEffect.builder().with(FireworkEffect.Type.BALL_LARGE).withColor(rainbowColorList[rainbowBlockIndex]).build(), psychic.plugin)
